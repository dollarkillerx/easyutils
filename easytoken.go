...
func EasyJwtGeneraToken(data *EasyJwtPayload,hour int) (string,error) {
	...
	// 签名
	signature, e := RsaSignSimple(payloadEnco, EasyJwtUtils.priKey) // 签名
	if e != nil {
		log.Println(e.Error())
		return "", e
	}
	...
	// 存入内存中
	EasyJwtUtils.data.Store(jwt,data)
...
func EasyJwtVerification(jwt string) error {
	value, ok := EasyJwtUtils.data.Load(jwt)
	if !ok {
